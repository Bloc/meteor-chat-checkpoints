...
The `{{else}}` renders a `loginPage` partial when a user isn't logged in. Let's create that template and add some styling so it's design is simple, but consistent with the rest of our application.
